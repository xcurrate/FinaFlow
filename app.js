...
async function deleteAllData() {
    ...
    if(err1 || err2) showToast("Terjadi kesalahan penghapusan.");
    else {
        showToast("Factory Reset Berhasil. Data bersih.");
        location.reload();
    }
}
...
async function addTransaction() {
    ...
    if(!amount) return showToast("Nominal wajib diisi");
...
async function addPlan() {
    ...
    if(!item) return showToast("Isi nama barang!");
...
async function login() { 
    ...
    if(error) {
        showToast("Gagal Masuk: Periksa kembali Username/Email dan Sandi.");
    } else { 
...
async function register() {
    ...
    if(error) showToast(error.message); else showToast("Cek Email!");
}
